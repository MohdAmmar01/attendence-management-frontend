...
const Login = () => {
    ...
    const handleSubmit = async (e) => {
      ...
      setloading(true)
            ...
            const res = await axios.post('https://attendence-management-mern-backend.onrender.com/api/user/login', { email , password })
            if (res.data.success) {
                setloading(false)
        ...
        <>
            <div className="login">
                ...
                <form className="login-form">
                    ...
                    <input id='mypassword' onChange={(e) => { setpassword(e.target.value) }} className='login-input' valuee={password} type='password' autoComplete='off' placeholder="Top secret" />
                    ...
                    <h4 style={{ 'color': 'black', 'fontFamily': 'cursive','textAlign': 'center' }} className='r-h'> Dont have an account ?<Link to='/register' style={{ 'color': 'black','fontSize':'18px', 'textDecoration': 'none' }}> Register</Link ></h4>
                </form>
                </div>
            </div>
            <ToastContainer />
